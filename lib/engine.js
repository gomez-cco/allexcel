...
const SYNONYMS = [
  ...
  { match: ['promedio si', 'promedio condicional', 'promedio por'], boost: ['PROMEDIO.SI'] },
  { match: ['si entonces', 'entonces', 'de lo contrario', 'en caso contrario', 'si es mayor', 'si es menor', 'mayor que', 'menor que', 'aprobado', 'rechazado'], boost: ['SI'] },
  ...
  { match: ['prestamo', 'cuota mensual', 'interes'], boost: ['PAGO', 'TASA', 'VA', 'VF'] },
];
...
function expandArgDefs(argDefs, count) {
  ...
  if (expanded.length < count && lastPairParts) {
    ...
    while (expanded.length < count) {
      ...
      expanded.push({ name: bumpTrailingNumber(part, repeatNum), desc: lastPairDesc });
      extra += 1;
    }
  }
  return expanded;
